...
| 特性            | 说明                                    |
| ------------- | ------------------------------------- |
| 🎧 **多格式兼容**  | 完美支持 MP3、FLAC、WAV 音频文件，自动识别文件类型       |
...
| 参数                     | 简写   | 类型      | 说明                                  |
| ---------------------- | ---- | ------- | ----------------------------------- |
| `--file-or-dir <PATH>` | `-f` | 路径      | 必选（与 `--list` 二选一）：单个音频文件或音乐目录路径    |
| `--list <PATH>`        | -    | 路径      | 必选（与 `-f` 二选一）：TXT 播放列表路径（一行一个文件路径） |
| `--clean`              | `-c` | 开关      | 可选：启用纯净模式，隐藏程序说明与控制指南               |
| `--mode <N>`           | `-m` | 数字（1-3） | 可选：播放模式（1 = 顺序，2 = 倒序，3 = 随机），默认 1  |
| `--loop`               | -    | 开关      | 可选：启用循环播放，播放列表结束后自动从头开始             |
| `--help`               | `-h` | 开关      | 可选：显示完整参数说明                         |
| `--version`            | `-V` | 开关      | 可选：显示当前版本（如 v1.0.0）                 |
...
1. 若播放列表中部分文件无法播放，可能是格式不支持（目前仅支持 MP3/FLAC/WAV）
